...
export interface IDataGridContextProps<TData extends object> {
  ...
  totalRows: number;
  loading: boolean;
  setQuerySearch: (string: string) => void;
...
export const DataGridProvider = <TData extends object>(props: TDataGridProps<TData>) => {
  ...
  const [querySearch, setQuerySearch] = useState<string>(''); // Add state for keyword
  ...
  }, [loading, pagination, sorting, columnFilters, querySearch, mergedProps.onFetchData]);
  ...
  useEffect(() => {
    loadData();
  }, [pagination, sorting, columnFilters, querySearch, mergedProps.data, mergedProps.serverSide]);
